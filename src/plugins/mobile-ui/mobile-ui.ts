...
import {
    ...
    setCachedAppId,
} from '@plugins/mobile-ui/appium-helpers';
...
function resolveAppPath(envVar: string | undefined): string | undefined {
    ...
    if (!fs.existsSync(resolved)) {
        logger.warn({ path: resolved }, '[Mobile-UI] App file not found — check ANDROID_APP_PATH / IOS_APP_PATH');
    }
    return resolved;
}
...
function loadCapabilities(sessionId: string = '0'): Record<string, unknown> {
    if (!CAP_PROFILE) {
        throw new Error(
            '[Mobile-UI] CAP_PROFILE env var is required. ' +
    ...
    if (!fs.existsSync(capPath)) {
        throw new Error(
            `[Mobile-UI] Capability profile not found: ${capPath}\n` +
    ...
    logger.info({ profile: CAP_PROFILE, platform: PLATFORM, sessionId, udid: udid ?? 'auto' }, '[Mobile-UI] Capabilities loaded');
    return caps;
}
...
async function ensureSession(sessionId: string): Promise<Browser> {
    ...
    logger.info({ sessionId, platform: PLATFORM }, '[Mobile-UI] Bootstrapping session...');
    const driver = await remote(wdioOptions);
    sessions.set(sessionId, driver);
    logger.info({ sessionId, total: sessions.size }, '[Mobile-UI] Session created');
    return driver;
}
...
async function teardown(sessionId: string): Promise<void> {
    ...
    if (driver) {
        ...
        logger.info(`[Mobile-UI] Session "${sessionId}" closed (remaining: ${sessions.size})`);
    }
}
...
export function getActiveDriver(sessionId: string = '0'): Browser {
    ...
    if (!driver) {
        throw new Error(
            `[Mobile-UI] No active session for sessionId='${sessionId}'. ` +
...
export async function teardownAllSessions(): Promise<void> {
    ...
    logger.info('[Mobile-UI] All sessions closed');
}

const registry = getMobileUiActionRegistry();
